-- Coluna casos.descricao + busca ILIKE-only (sem similaridade/trigrama).
-- Descrição livre do caso; correspondência apenas por contains com unaccent.

ALTER TABLE public.casos
  ADD COLUMN IF NOT EXISTS descricao text;

COMMENT ON COLUMN public.casos.descricao IS
  'Descrição livre opcional do caso (texto livre).';

-- ---------------------------------------------------------------------------
-- Match ILIKE-only (contains) com unaccent — sem similaridade/trigrama
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.busca_match_ua_contains(
  valor text,
  termo_ua text
)
...
COMMENT ON FUNCTION public.busca_match_ua_contains(text, text) IS
  'Match ILIKE contains com unaccent (sem similaridade). Retorna tipo_corr=exata, score=1.0.';

GRANT EXECUTE ON FUNCTION public.busca_match_ua_contains(text, text)
  TO authenticated;
...
COMMENT ON FUNCTION public.busca_global(text, double precision, integer) IS
  'Busca global exata/aproximada. Nomes de pessoa: ILIKE + trgm(0.3) + fonética PT-BR. casos.descricao via ILIKE. SECURITY INVOKER (RLS).';
