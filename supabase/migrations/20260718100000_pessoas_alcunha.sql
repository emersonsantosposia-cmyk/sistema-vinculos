-- Campo alcunha em pessoas + Ã­ndices e inclusÃ£o na busca_global

ALTER TABLE public.pessoas
  ADD COLUMN IF NOT EXISTS alcunha text;

COMMENT ON COLUMN public.pessoas.alcunha IS
  'Alcunha, apelido ou nome de guerra da pessoa.';

CREATE INDEX IF NOT EXISTS pessoas_alcunha_trgm_idx
  ON public.pessoas USING gin (alcunha gin_trgm_ops);

CREATE INDEX IF NOT EXISTS pessoas_alcunha_unaccent_trgm_idx
  ON public.pessoas USING gin (public.immutable_unaccent(alcunha) gin_trgm_ops);
...
AS $$
  ...
  candidatos AS (
    ...
    SELECT
      ...
      p.id AS entidade_id,
      coalesce(nullif(btrim(p.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o') AS rotulo_principal,
    ...
    -- Redes sociais Ã¢â€ â€™ resultado como pessoa
    SELECT
      'pessoa',
      p.id,
      coalesce(nullif(btrim(p.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo,
      m.tipo_corr,
      m.score
    ...
    SELECT
      'empresa', e.id,
      coalesce(nullif(btrim(e.nome_fantasia), ''), nullif(btrim(e.razao_social), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
    ...
    SELECT
      'endereco', en.id,
      coalesce(nullif(btrim(en.nome), ''), nullif(btrim(en.logradouro), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    ...
    SELECT
      'veiculo', v.id,
      coalesce(nullif(btrim(v.placa), ''), nullif(btrim(concat_ws(' ', v.marca, v.modelo)), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    ...
    SELECT
      'procedimento', pr.id,
      coalesce(nullif(btrim(pr.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    ...
    SELECT
      'caso', c.id,
      coalesce(nullif(btrim(c.numero), ''), nullif(btrim(c.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    FROM public.casos c
    CROSS JOIN params par
    CROSS JOIN LATERAL (
    ...
    SELECT
      'comunicacao', co.id,
      coalesce(nullif(btrim(co.valor), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    ...
    SELECT
      'orcrim', o.id,
      coalesce(nullif(btrim(o.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
    ...
    SELECT
      'usuario', u.id,
      coalesce(nullif(btrim(u.nome), ''), 'Sem identificaÃƒÂ§ÃƒÂ£o'),
      m.campo, m.tipo_corr, m.score
...
COMMENT ON FUNCTION public.busca_global(text, double precision, integer) IS
  'Busca global exata/aproximada em todos os campos textuais das entidades (incl. alcunha). SECURITY INVOKER (RLS).';
