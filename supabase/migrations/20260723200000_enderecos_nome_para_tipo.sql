-- enderecos: substitui coluna nome por tipo (texto livre com sugestões na UI).
-- 1) adiciona tipo  2) copia nome → tipo  3) atualiza busca_global  4) índices  5) remove nome

ALTER TABLE public.enderecos
  ADD COLUMN IF NOT EXISTS tipo text;

COMMENT ON COLUMN public.enderecos.tipo IS
  'Tipo do endereço (ex.: casa, apartamento, empresa). Texto livre; a UI sugere valores comuns.';

UPDATE public.enderecos
SET tipo = nullif(btrim(nome), '')
WHERE tipo IS NULL AND nome IS NOT NULL;
...
AS $$
  ...
  candidatos AS (
    ...
    SELECT
      'endereco', en.id,
      coalesce(
        nullif(btrim(concat_ws(', ', nullif(btrim(en.logradouro), ''), nullif(btrim(en.numero), ''))), ''),
        nullif(btrim(en.tipo), ''),
        'Sem identificação'
      ),
      m.campo, m.tipo_corr, m.score
    ...
    CROSS JOIN LATERAL (
      SELECT * FROM (
        SELECT 'tipo'::text, x.* FROM public.busca_match_ua(en.tipo, par.t_ua, par.thr) x
        UNION ALL
        SELECT 'logradouro', x.*
    ...
    CROSS JOIN LATERAL (
      SELECT * FROM (
        ...
        UNION ALL
        SELECT 'descricao', x.* FROM public.busca_match_ua_contains(c.descricao, par.t_ua) x
        UNION ALL
...
COMMENT ON FUNCTION public.busca_global(text, double precision, integer) IS
  'Busca global. Endereços usam campo tipo (ex-nome); rótulo principal = logradouro+número. SECURITY INVOKER (RLS).';
...
-- ---------------------------------------------------------------------------
-- Índices trgm: nome → tipo
-- ---------------------------------------------------------------------------
DROP INDEX IF EXISTS public.enderecos_nome_trgm_idx;
DROP INDEX IF EXISTS public.enderecos_nome_unaccent_trgm_idx;

CREATE INDEX IF NOT EXISTS enderecos_tipo_trgm_idx
  ON public.enderecos USING gin (tipo gin_trgm_ops);

CREATE INDEX IF NOT EXISTS enderecos_tipo_unaccent_trgm_idx
  ON public.enderecos USING gin (public.immutable_unaccent(tipo) gin_trgm_ops);

ALTER TABLE public.enderecos DROP COLUMN IF EXISTS nome;
